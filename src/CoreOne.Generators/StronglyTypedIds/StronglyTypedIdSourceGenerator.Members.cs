...
public partial class StronglyTypedIdSourceGenerator
{
    ...
    private static void GenerateTypeMembers(CSharpFileWriter writer, AttributeInfo context)
    {
        ...
        if (!context.IsEmptyDefined && context.IdType is IdType.System_String or IdType.System_Guid or IdType.System_String)
        {
            WriteNewMember(
             XmlSummary("Default empty instance."),
             XmlReturn("The value contained in this instance."));
            writer.WriteLine($"public static {context.TypeName} {PropertyEmpty} {{ get; }} = {ValueToStringExpression()};");
            ...
            string ValueToStringExpression()
            {
                ...
                if (context.IdType is IdType.System_Boolean)
                    return $"false";

                // if (context.IdType == IdType.System_Guid)
                return $"new {context.TypeName}(Guid.Empty)";
            }
        }
        ...
        //  if (!context.IsDefaultCtorDefined)
        {
            ...
            using (writer.BeginBlock($"public {context.TypeName}()"))
            {
        ...
        using (writer.BeginBlock($"public static implicit operator {context.TypeName}({context.ValueTypeCSharpTypeName} value)"))
        {
            writer.WriteLine($"return new {context.TypeName}(value);");
        }
        ...
        WriteNewMember(
            ...
            XmlParam("value", "The value to create the instance."),
            XmlReturn("A new instance of ", XmlSeeCref(context.TypeName), "."));
        using (writer.BeginBlock($"public static {context.TypeName} From{context.ValueTypeShortName}({context.ValueTypeCSharpTypeName} value)"))
        {
            writer.WriteLine($"return new {context.TypeName}(value);");
        }
        ...
        if (!context.IsToStringDefined)
        {
            ...
            using (writer.BeginBlock("public override string ToString()"))
            {
                ...
                {
                    writer.WriteLine($"return {PropertyAsStringName} ?? string.Empty;");
                }
            }
        }
        ...
        if (context.IdType is IdType.System_Guid)
        {
            WriteNewMember(
                 ...
                 XmlReturn("A new instance of ", XmlSeeCref(context.TypeName), "."));
            using (writer.BeginBlock($"public static {context.TypeName} New()"))
            {
                writer.WriteLine($"return new {context.TypeName}(global::System.Guid.NewGuid());");
            }

            WriteNewMember(
                 XmlSummary("Initializes a new instance of the ", XmlSeeCref(context.TypeName), " using a new ", XmlSeeCref("global::System.Guid"), "."),
                 XmlReturn("A new instance of ", XmlSeeCref(context.TypeName), "."));
            using (writer.BeginBlock($"public static {context.TypeName} Create()"))
            {
                writer.WriteLine($"return new {context.TypeName}(global::CoreOne.ID.Create().AsGuid());");
            }
        }
        ...
        void GenerateTryParseMethod(CSharpFileWriter writer, AttributeInfo context, bool isReadOnlySpan)
        {
